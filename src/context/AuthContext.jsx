...
export const Profile = () => {
  // Removed 'login' to fix the ESLint no-unused-vars warning
  ...
  // HEATMAP GENERATOR wrapped in useMemo to fix React purity rule and avoid Math.random during render
  const heatmap = useMemo(() => {
    ...
    for (let w = 0; w < weeks; w++) {
      ...
      for (let d = 0; d < daysPerWeek; d++) {
        ...
        let intensity; // Fixed: Removed useless assignment of 0
        
        if (daysAgo < streak) {
           // Fixed: Replaced Math.random() with deterministic logic
           intensity = (daysAgo % 3) + 2; // Alternates between 2, 3, and 4
        } else if (daysAgo > 111) {
           intensity = 0;
        } else {
